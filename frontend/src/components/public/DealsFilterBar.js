  ...
  return (
    <div className="flex flex-wrap items-center gap-6 justify-between" data-testid="deals-filter-bar">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="inline-flex items-center bg-transparent border border-[var(--border-default)] rounded-md h-[45px] px-1">
            ...
            return (
              <button
                ...
                className={`h-[37px] w-[54px] flex items-center justify-center rounded transition-colors ${
                  active ? 'bg-[var(--accent-brand)] text-black' : 'text-[var(--text-primary)] hover:text-[var(--accent-brand)]'
                }`}
              >
                {isMask ? (
                  <Icon size={20} color={active ? '#F5F0E8' : '#17202A'} />
                ) : (
                  <Icon size={20} />
                )}
              </button>
        ...
        <div className="inline-flex items-center bg-transparent border border-[var(--border-default)] rounded-md h-[45px] overflow-hidden">
          {PRICES.map((p) => (
            <button
              key={p.id}
              type="button"
              ...
              className={`h-full px-6 text-[13px] font-medium uppercase tracking-wide transition-colors ${
                price === p.id ? 'bg-[var(--accent-brand)] text-black' : 'text-[var(--text-primary)] hover:text-[var(--accent-brand)]'
              }`}
            >
              {p.label}
            </button>
