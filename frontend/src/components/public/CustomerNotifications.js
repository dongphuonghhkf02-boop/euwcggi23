...
export const CustomerNotificationsPage = ({ customerId, customerToken }) => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="w-8 h-8 border-2 border-[var(--border-default)] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6" data-testid="customer-notifications-page">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                filter === 'all'
                  ...
                  : 'bg-[#F4F4F5] text-[var(--text-muted)] hover:bg-[#E4E4E7]'
              }`}
      ...
      <div className="bg-gradient-to-r from-[#0088cc] to-[#229ED9] text-[var(--text-primary)] rounded-2xl p-5">
        <div className="flex items-center gap-4">
          <TelegramLogo size={40} weight="fill" />
          <div className="flex-1">
            <h3 className="font-semibold">Отримуйте сповіщення в Telegram</h3>
            <p className="text-[var(--text-primary)]/80 text-sm mt-1">
              Миттєві сповіщення про аукціони та зниження цін
            </p>
          </div>
          <a
            href="https://t.me/DmAutoCarsBot"
            target="_blank"
      ...
      {notifications.length > 0 ? (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className={`font-semibold ${notification.isRead ? 'text-[var(--text-muted)]' : 'text-[var(--text-primary)]'}`}>
                          {notification.title}
                        </h3>
        ...
        <div className="bg-white border border-[#E4E4E7] rounded-2xl p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">Немає сповіщень</h3>
          <p className="text-[var(--text-muted)] mt-1">
...
export const NotificationBell = ({ customerToken, onClick }) => {
  ...
  return (
    ...
    >
      ...
      {unreadCount > 0 && (
        <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-red-500 text-[var(--text-primary)] text-xs font-medium rounded-full flex items-center justify-center px-1">
