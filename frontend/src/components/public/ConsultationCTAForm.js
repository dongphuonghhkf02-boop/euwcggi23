...
export const ConsultationCTAForm = () => {
  ...
  const Field = ({ label, children }) => (
    <label className="block mb-6">
      <span className="block text-[14px] text-[var(--text-primary)] mb-3">{label}</span>
      {children}
    </label>
  );

  return (
    <section className="bg-[var(--bg-elevated)] py-[100px]" data-testid="consultation-section">
      <div className="max-w-[1920px] mx-auto px-6 lg:px-[100px] text-center">
        <h2 className="text-[56px] md:text-[80px] font-bold uppercase text-[var(--accent-brand)] leading-tight">
          Don't postpone buying a car
        </h2>
        <h3 className="text-[48px] md:text-[80px] font-bold text-[var(--text-primary)] leading-tight mb-16">Free consultation</h3>
