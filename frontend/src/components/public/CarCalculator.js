...
import DmAutoSelect from '../ui/DmAutoSelect';
...
const FALLBACK_ORIGINS = [
  { code: 'usa', name: 'Europe → Belarus' },
  { code: 'korea', name: 'Europe → Russia' },
];
...
const FALLBACK_AUCTIONS = [
  { code: 'copart', name: 'Mobile.de' },
  { code: 'iaai', name: 'BCA Europe' },
];
...
const DottedRow = ({ label, value, valueClass = 'text-[var(--text-primary)]', bold = false }) => (
  <div className="flex items-baseline gap-3 py-[11px]">
    <span className={`text-[13px] md:text-[15px] text-[var(--text-primary)] whitespace-nowrap ${bold ? 'font-semibold' : ''}`}>
      {label}
    </span>
    <span
      aria-hidden="true"
...
const FieldInput = ({ label, value, onChange, placeholder, type = 'text', testId, suffix }) => (
  <div className="mb-5">
    <label className="block text-[12px] uppercase tracking-wider text-[var(--text-muted)] mb-2">{label}</label>
    <div className="relative">
      <input
        ...
        type={type}
        data-testid={testId}
        className="w-full h-12 bg-transparent border border-[var(--border-default)] rounded px-4 pr-14 text-[14px] text-[var(--text-primary)] placeholder-[#6A6A6A] focus:outline-none focus:border-[var(--accent-brand)]"
      />
      {suffix ? (
        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[12px] uppercase tracking-wider text-[var(--accent-brand)] pointer-events-none">
          {suffix}
        </span>
      ) : null}
    </div>
...
export const CarCalculator = ({ vehicle, initialVin = '', initialPrice = null }) => {
  ...
  // Build options for DmAutoSelect
  ...
  return (
    <section
      className="bg-[var(--bg-elevated)] rounded-lg px-6 py-12 md:px-16 md:py-16"
      data-testid="car-calculator"
    >
      <div className="flex items-center justify-between mb-10 gap-4 flex-wrap">
        <h3 className="text-[28px] md:text-[40px] font-bold text-[var(--text-primary)]">Car calculator</h3>
        <div className="flex items-center gap-3 text-[12px] uppercase tracking-wider text-[var(--text-muted)]">
          {loading ? (
            <span className="inline-flex items-center gap-2 text-[var(--accent-brand)]">
          ...
          <button
            type="button"
            onClick={reset}
            className="underline underline-offset-4 hover:text-[var(--accent-brand)]"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-24">
        ...
        <div className="flex flex-col">
                ...
                <button
                  ...
                  className={`h-12 rounded border text-[12px] uppercase tracking-wider font-semibold transition-colors ${
                    origin === opt.value
                      ...
                      : 'bg-transparent text-[var(--text-primary)] border-[var(--border-default)] hover:border-[var(--accent-brand)]'
                  }`}
                >
                  {opt.label}
                </button>
          ...
          <div className="mt-2 border-t border-[var(--border-default)] pt-5">
            ...
            <div className="flex items-baseline gap-3 py-[14px]">
              <span className="text-[14px] md:text-[15px] text-[var(--text-primary)] whitespace-nowrap">Car delivered (BY / RU)</span>
              <span
                aria-hidden="true"
                className="flex-1 self-end mb-[5px]"
              ...
              />
              <span
                className="text-[18px] md:text-[22px] font-bold uppercase text-[var(--accent-brand)] whitespace-nowrap"
          ...
          {lastQuoteId ? (
            <div className="mt-3 text-[12px] text-[var(--text-muted)]">
              Quote saved:{' '}
              <span className="text-[var(--accent-brand)] font-mono" data-testid="calc-quote-id">
                {lastQuoteId}
              </span>
            </div>
          ) : null}
          ...
          {/* Phone */}
          <label className="block text-[14px] text-[var(--text-primary)] mt-10 mb-3">
            Your phone number<span className="text-[var(--accent-brand)]">*</span>
          </label>
          <div className="relative">
            <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-2 pr-3 border-r border-[var(--border-default)] h-6">
              <span
                className="inline-block w-6 h-4 rounded-sm overflow-hidden"
                aria-label="Belarus / Russia"
                style={{
                  background:
            ...
            <input
              ...
              placeholder="+359 ..."
              type="tel"
              className="w-full h-12 bg-transparent border border-[var(--border-default)] rounded pl-[70px] pr-4 text-[14px] text-[var(--text-primary)] placeholder-[#6A6A6A] focus:outline-none focus:border-[var(--accent-brand)]"
          ...
          <button
            ...
            onClick={submitLead}
            disabled={submitting}
            className="w-full mt-4 h-[52px] border border-[var(--accent-brand)] text-[var(--accent-brand)] text-[13px] uppercase tracking-[0.2em] font-semibold hover:bg-[var(--accent-brand)] hover:text-black transition-colors rounded disabled:opacity-60 disabled:cursor-not-allowed"
          ...
          <button
            type="button"
            className="mt-10 self-start text-[13px] md:text-[14px] font-medium uppercase underline text-[var(--accent-brand)] hover:brightness-110 tracking-wider"
        ...
        <div className="flex flex-col">
          <div className="text-[12px] uppercase tracking-[0.2em] text-[var(--accent-brand)] mb-4">
            [ {isKorea ? 'europe → poland → russia' : 'turnkey breakdown'} ]
          </div>
          {breakdown.length === 0 ? (
            <div className="text-[14px] text-[var(--text-muted)] italic py-10">
          ...
          {breakdown.length > 0 ? (
            <div className="mt-8 pt-6 border-t border-[var(--border-default)]">
              <DottedRow
                label="Total (delivered BY / RU)"
                value={fmt(total, currency)}
                valueClass="text-[var(--accent-brand)] text-[18px] md:text-[20px]"
                bold
              />
              {isKorea && calc?.totalEur ? (
                <DottedRow
                  ...
                  valueClass="text-[var(--text-primary)] text-[14px]"
                />
              ) : null}
