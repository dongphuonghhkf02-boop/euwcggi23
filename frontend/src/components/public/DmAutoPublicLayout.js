/**
 * DM Auto — shared public-page Layout (Header + Footer).
 *
 ...
 * `<ScaledChrome />`. There used to be a parallel "legacy" DM Auto header/footer
...
import './DmAutoPublicLayout.css';

export function DmAutoHeader() {
...
export function DmAutoFooter() {
  const isMobile = useIsMobile(768);
  const { pathname } = useLocation();
  // Same rationale as DmAutoHeader: on mobile homepage the MobileHomePage
...
export default { DmAutoHeader, DmAutoFooter };
