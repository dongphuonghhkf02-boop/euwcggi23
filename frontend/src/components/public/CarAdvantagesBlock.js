...
const ADVANTAGES = [
  ...
  },
  {
    ...
    text: 'Full VIN reports (AutoDNA, Mobile.de) — you see the real story before you bid.',
  },
];

const Bullet = ({ title, text }) => (
  <div>
    <div className="text-[18px] md:text-[20px] font-bold text-[var(--text-primary)]">
      <span className="text-[var(--accent-brand)] mr-2">/</span>
      {title}
    </div>
    <p className="mt-3 text-[14px] md:text-[15px] text-[var(--text-muted)] leading-relaxed max-w-[340px]">
      {text}
    </p>
  </div>
);
...
export default function CarAdvantagesBlock() {
  return (
    <section className="bg-[var(--bg-base)] py-24 relative overflow-hidden" data-testid="car-advantages-section">
      <div className="max-w-[1920px] mx-auto px-6 lg:px-[100px] relative">
        ...
        <div className="text-center mb-16">
          <div className="font-bold uppercase tracking-[-0.01em] leading-[1.1]" style={{ fontSize: 'clamp(26px, 2.6vw, 40px)' }}>
            <span className="text-[var(--accent-brand)]">Why you pay less</span>
            <span className="text-[var(--text-primary)]"> — and get more</span>
          </div>
        </div>
        ...
        <div className="relative" style={{ minHeight: '520px' }}>
          ...
          <div
            className="absolute left-0 top-0 font-black uppercase text-[var(--text-primary)] leading-[0.85] pointer-events-none select-none"
          ...
          <div className="relative pt-[220px] md:pt-[260px] grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-10 lg:gap-12 items-start">
            ...
            <div className="hidden lg:flex w-[320px] justify-center items-center">
              ...
              >
                ...
                {/* Corner brackets */}
                <span className="absolute top-2 left-2 w-6 h-6 border-l-2 border-t-2 border-[var(--accent-brand)]" />
                <span className="absolute top-2 right-2 w-6 h-6 border-r-2 border-t-2 border-[var(--accent-brand)]" />
                <span className="absolute bottom-2 left-2 w-6 h-6 border-l-2 border-b-2 border-[var(--accent-brand)]" />
                <span className="absolute bottom-2 right-2 w-6 h-6 border-r-2 border-b-2 border-[var(--accent-brand)]" />
              </div>
            </div>
