...
const ArrowLeftCircle = ({ disabled }) => (
  <svg width="40" height="40" viewBox="0 0 40 40" aria-hidden="true">
    <circle cx="20" cy="20" r="19.5" stroke={disabled ? '#5E5E5E' : '#162E51'} fill="none" />
    <path d="M23 13 16 20l7 7" stroke={disabled ? '#5E5E5E' : '#162E51'} strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
const ArrowRightCircle = ({ disabled }) => (
  <svg width="40" height="40" viewBox="0 0 40 40" aria-hidden="true">
    <circle cx="20" cy="20" r="19.5" fill={disabled ? 'transparent' : '#162E51'} stroke={disabled ? '#5E5E5E' : '#162E51'} />
    <path d="M17 13l7 7-7 7" stroke={disabled ? '#5E5E5E' : '#FFFFFF'} strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
const ArrowRightSm = () => (
      ...
      <div className={styles.page}>
        <div className={singleStyles.shell}>
          <div className={singleStyles.skeleton}>
            {tLocale === 'ru' ? 'Загрузка статьи…' : 'Loading article…'}
          </div>
        </div>
      </div>
    );
      ...
      <div className={styles.page}>
        <div className={singleStyles.shell}>
          <div className={singleStyles.notFound}>
            <h1>{tLocale === 'ru' ? 'СТАТЬЯ НЕ НАЙДЕНА' : 'ARTICLE NOT FOUND'}</h1>
            <p>
              {tLocale === 'ru'
            ...
            <button onClick={() => navigate('/blog')} className={singleStyles.backBtn} data-testid="blog-back-btn">
              ← {tLocale === 'ru' ? 'НАЗАД К БЛОГУ' : 'BACK TO BLOG'}
            </button>
          </div>
        </div>
      </div>
          ...
          <div className={singleStyles.metaRow}>
            ...
            <div className={singleStyles.dateRow}>
              ...
              <span className={singleStyles.minRead}>
                {article.read_time_minutes} {tLocale === 'ru' ? 'мин. чтения' : 'min read'}
              </span>
            </div>
          </div>
        </header>
        ...
        <section className={singleStyles.interestWrap}>
          <div className={singleStyles.interestInner}>
            <div className={singleStyles.interestHeader}>
              ...
              >
                {tLocale === 'ru' ? 'ВСЕ СТАТЬИ' : 'VIEW ALL ARTICLES'}
                <ArrowRightSm />
              </button>
            </div>
                  ...
                  >
                    ...
                    <div className={singleStyles.relatedMeta}>
                      ...
                      <div className={singleStyles.dateRow}>
                        ...
                        <span className={singleStyles.minRead}>
                          {r.read_time_minutes} {tLocale === 'ru' ? 'мин.' : 'min read'}
                        </span>
                      </div>
                    </div>
        ...
        <div className={singleStyles.ctaInner}>
          <div className={singleStyles.ctaText}>
            <h3 className={singleStyles.ctaSmallTitle}>
              {tLocale === 'ru' ? 'ГОТОВЫ ИМПОРТИРОВАТЬ АВТОМОБИЛЬ?' : 'READY TO IMPORT YOUR CAR?'}
            </h3>
            <div className={singleStyles.ctaBigTitle}>
              <div>{tLocale === 'ru' ? 'ХВАТИТ ЧИТАТЬ.' : 'STOP READING.'}</div>
              <div className={singleStyles.accent}>{tLocale === 'ru' ? 'НАЧНИ ЕЗДИТЬ.' : 'START DRIVING.'}</div>
            </div>
            <p className={singleStyles.ctaDesc}>
              {tLocale === 'ru'
                ? 'Наши менеджеры подберут подходящий автомобиль под ваш бюджет, возьмут на себя весь процесс импорта и доставят его прямо к вашему порогу. Без скрытых платежей — только понятная цена с первого дня.'
          ...
          <div className={singleStyles.ctaButtons}>
            ...
            >
              {tLocale === 'ru' ? 'ЗАКАЗАТЬ АВТО' : 'REQUEST A VEHICLE'}
            </button>
            <button
              type="button"
            ...
            >
              {tLocale === 'ru' ? 'ИСПОЛЬЗОВАТЬ КАЛЬКУЛЯТОР' : 'USE OUR CALCULATOR'}
            </button>
          </div>
        </div>
      </section>
