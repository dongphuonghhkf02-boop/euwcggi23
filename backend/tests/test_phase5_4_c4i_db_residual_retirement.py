...
def test_1_c4i_files_no_longer_import_db_from_server():
    ...
    assert not bad, (
        "[C-4i] FAIL: files still import `db` from server:\n"
...
def test_2_only_di_source_remains():
    ...
    if sites == []:
        print("✓ test_2_only_di_source_remains  (post-C-4j: 0 bridges)")
    else:
...
def test_3_qualified_server_db_access_retired():
    ...
    assert not hits, (
        "[C-4i] FAIL: qualified `server.db` access still present at:\n"
        + "\n".join(f"  {h}" for h in hits)
    )
    print("✓ test_3_qualified_server_db_access_retired  (0 references; was 47)")
...
def test_4_qualified_import_inventory_empty():
    ...
    print("✓ test_4_qualified_import_inventory_empty  (() — fully retired)")
...
def test_6_identity_runtime_sentinel_rebind():
    ...
    print("✓ test_6_identity_runtime_sentinel_rebind  (2-step rebind observed)")
...
def test_8_prior_batches_remain_migrated():
    ...
    print("✓ test_8_prior_batches_remain_migrated  "
          "(C-4e=12, C-4f=4, C-4g=1, C-4h=4)")
...
def test_9_c4i_inventory_flipped_to_migrated():
    ...
    print("✓ test_9_c4i_inventory_flipped_to_migrated  "
          "(4/4 from-import entries migrated=True; 2 qualified-import "
          "files retired via DB_QUALIFIED_IMPORT_SITES path)")
...
def test_10_bridge_inventory_unchanged():
    ...
    label = "post-C-4j (db retired, Tier-A empty)" if valid_post else "pre-C-4j (db present, Tier-A={db})"
