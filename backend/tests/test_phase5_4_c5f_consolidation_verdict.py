...
def test_3_tier_a_empty():
    ...
    print("✓ test_3: Tier-A == frozenset() (held since C-4j)")
...
def test_4_tier_b_inventory_is_static_dir_only():
    ...
    print("✓ test_4: TIER_B == {'_STATIC_DIR'} (both inventories)")
...
def test_6_retired_sets_disjoint_from_live_imports():
    ...
    assert not regressions, (
        "[C-5f] FAIL: retired symbols have live production "
        "`from server import` sites:\n" + "\n".join(regressions)
    )
...
def test_7_static_dir_target_phase_5_8():
    ...
    print("✓ test_7: _STATIC_DIR targeted at Phase 5.8 in all 3 "
          "declarations")
...
def test_10_no_production_code_changed_in_c5f():
    ...
    assert all(hasattr(b, "symbol") and hasattr(b, "tier")
               for b in BRIDGE_INVENTORY)
    print("✓ test_10: no production code change (4 probes — server "
          "shims + canonical owner + 4 accessor modules + inventory "
          "identity)")
