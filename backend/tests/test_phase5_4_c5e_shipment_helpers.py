...
def test_1_no_prod_from_server_import_get_current_stage():
    ...
    assert not prod, (
        "[C-5e] FAIL: `get_current_stage` still imported via "
        "`from server import` in production code:\n  " + "\n  ".join(prod)
    )
    print("✓ test_1: 0 production `from server import get_current_stage` sites")
...
def test_2_no_prod_from_server_import_serialize_journey():
    ...
    assert not prod, (
        "[C-5e] FAIL: `serialize_journey` still imported via "
        "`from server import` in production code:\n  " + "\n  ".join(prod)
    )
    print("✓ test_2: 0 production `from server import serialize_journey` sites")
...
def test_3_canonical_imports_app_utils_shipments():
    ...
    print("✓ test_3: canonical owner = app/utils/shipments.py; both symbols "
          "public, callable, and imported in production")
...
def test_4_behaviour_parity_get_current_stage():
    ...
    print("✓ test_4: get_current_stage parity (6 cases × canon+shim = 12 asserts)")
...
def test_5_behaviour_parity_serialize_journey():
    ...
    print("✓ test_5: serialize_journey parity ("
          "28-field shape × 6 cases; trackingHealth=4-bucket; emotionalText)")
...
def test_7_canonical_module_no_orchestration_imports():
    ...
    print("✓ test_7: app/utils/shipments.py = pure utility (no db/sio/audit/await)")
...
def test_8_bridge_inventory_delta_correct():
    ...
    print("✓ test_8: inventory delta correct "
          "(BRIDGE 13→11, TIER_B 3→1, both symbols retired)")
