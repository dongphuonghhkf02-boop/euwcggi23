...
def test_1_no_remaining_from_server_import_db():
    ...
    assert sites == [], (
        "[C-4j] FAIL: `from server import db` still present at:\n"
        + "\n".join(f"  {s}" for s in sites)
    )
    print("✓ test_1_no_remaining_from_server_import_db  "
          "(production AST grep == 0)")
...
def test_2_no_qualified_server_db_access():
    ...
    assert hits == [], (
        "[C-4j] FAIL: qualified `server.db.<attr>` access still "
        "present at:\n" + "\n".join(f"  {h}" for h in hits)
    )
    print("✓ test_2_no_qualified_server_db_access  "
          "(production AST grep == 0)")
...
def test_4_deps_get_db_delegates_to_db_runtime():
    ...
    try:
        ...
        assert deps.get_db() is sentinel, (
            "[C-4j] FAIL: deps.get_db() did not return the sentinel "
            "published via db_runtime.set_db. The DI source is not "
            "routing through the accessor."
        )
    finally:
        db_runtime.set_db(saved_db, saved_client)
    print("✓ test_4_deps_get_db_delegates_to_db_runtime  "
          "(import present + sentinel round-trip OK)")
...
def test_5_post_startup_identity_chain_intact():
    ...
    with TestClient(fastapi_app) as client:
        ...
        assert a is b, "[C-4j] FAIL: server.db is NOT db_runtime.get_db()"
        assert b is c, "[C-4j] FAIL: db_runtime.get_db() is NOT deps.get_db()"
        assert a is c, "[C-4j] FAIL: server.db is NOT deps.get_db()"

    print("✓ test_5_post_startup_identity_chain_intact  "
          "(server.db ≡ db_runtime.get_db() ≡ deps.get_db(), all non-None)")
...
def test_7_tier_a_empty():
    ...
    print("✓ test_7_tier_a_empty  (Tier-A == frozenset())")
...
def test_10_qualified_import_inventory_empty():
    ...
    print("✓ test_10_qualified_import_inventory_empty  (() — fully retired)")
