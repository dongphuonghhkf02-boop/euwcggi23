...
def test_1_payments_has_no_server_logger_usage():
    ...
    print("✓ test_1: 0 `server.logger` qualified-access sites in "
          "payments.py (was 8 pre-5.5/A)")
...
def test_2_payments_has_module_local_logger():
    ...
    print("✓ test_2: module-local `logger = logging.getLogger(...)` "
          "present in payments.py + `import logging` present")
...
def test_5_no_payment_route_signatures_changed():
    ...
    assert not bad, (
        "[5.5/A] FAIL: route signatures degraded:\n  " + "\n  ".join(bad)
    )
