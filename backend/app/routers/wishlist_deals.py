...
# Admin-managed weekly picks: category is no longer surfaced on the public
# site (the homepage block lets the user filter by BUDGET only), so we make
# `category` optional everywhere. Cards added via the admin UI default to
# this neutral value to keep the legacy unique compound index happy.
DEFAULT_CATEGORY = "sedan"


def _is_admin_like(user: dict) -> bool:
    return (user.get("role") or "").lower() in {"admin", "owner", "master_admin", "team_lead"}
...
async def public_list(
    ...
    Ordering: explicit `display_order` (admin-set) ASC, then most recently
    added first. The admin "Подборка недели" UI lets the operator reorder
    the cards so that a hand-picked TOP-N appears first on the homepage.
    """
    db = _db()
...
async def manager_create(
    ...
    category = (payload.get("category") or "").strip() or DEFAULT_CATEGORY
    ...
    # Admin / team-lead / owner / master_admin → auto-approve so the card
    # shows on the public homepage block immediately. Regular managers
    # still go through the pending queue.
    auto_approve = _is_admin_like(user)
    # Compute next display_order so new cards land at the end of the
    # week's ranking by default. Admin can re-order via PATCH/reorder.
    try:
        last = await db.wishlist_deals.find_one(
            {"week_start": week_start},
            sort=[("display_order", -1)],
            projection={"display_order": 1},
        )
        next_order = int((last or {}).get("display_order") or 0) + 10
    except Exception:
        next_order = 1000
    doc = {
        "id": str(uuid.uuid4()),
        ...
        "created_at": now,
        "approved_by": (user.get("id") or user.get("_id") or user.get("user_id")) if auto_approve else None,
        "approved_by_name": (user.get("name") or user.get("email") or "admin") if auto_approve else None,
...
# ─────────────────────────────────────────────────────────── ADMIN
# Dedicated `/api/admin/wishlist-deals` namespace — used by the new
# admin UI ("Подборка недели"). Auto-approves new cards so they appear
# on the public homepage immediately. Russian-only UI on the frontend.

admin_router = APIRouter(
    prefix="/api/admin/wishlist-deals",
    tags=["admin-wishlist-deals"],
    dependencies=[Depends(require_admin)],
)


@admin_router.get("/vin-search")
async def admin_vin_search(
    q: str = Query("", description="VIN prefix or free-text (year/make/model)"),
    limit: int = Query(10, ge=1, le=25),
):
    """Same VIN autocomplete as manager_vin_search, exposed under /api/admin."""
    return await manager_vin_search(q=q, limit=limit)
...
async def admin_list(
    ...
    """List all wishlist items for the requested week (defaults to current).
    Used by the admin "Подборка недели" page to show the current curation."""
    db = _db()
    q: Dict[str, Any] = {}
    if week:
        q["week_start"] = _resolve_week(week)
    ...
    try:
        ...
        items = [_strip(d) async for d in cursor]
        # Counts per budget tier for the current week (status=approved) so
        # the admin UI can show "10–15K: 7 / 15–25K: 6 / 30–50K: 5".
        week_q = {"week_start": q.get("week_start", _resolve_week("current")), "status": "approved"}
        by_budget: Dict[str, int] = {b: 0 for b in VALID_BUDGETS}
        try:
            pipeline = [
                {"$match": week_q},
                {"$group": {"_id": "$budget", "n": {"$sum": 1}}},
            ]
            async for row in db.wishlist_deals.aggregate(pipeline):
                key = row.get("_id")
                if key in by_budget:
                    by_budget[key] = int(row.get("n") or 0)
        except Exception:
            pass
        return {
            "data": items,
            "week_start": q.get("week_start", _resolve_week("current")),
            "counts": by_budget,
            "total": len(items),
        }
    except Exception as e:
        logger.warning(f"[wishlist_deals/admin_list] {e}")
        return {"data": [], "week_start": _resolve_week("current"), "counts": {}, "total": 0}
...
async def admin_create(
    ...
    """Add a VIN to the weekly curation. Auto-approves so it shows on the
    public homepage immediately. Category is optional (defaults to 'sedan')."""
    return await manager_create(payload=payload, user=user)
...
async def admin_delete(
    ...
    """Remove an item from the weekly curation."""
    db = _db()
    res = await db.wishlist_deals.delete_one({"id": item_id})
    if not res.deleted_count:
        raise HTTPException(status_code=404, detail="Item not found")
    return {"deleted": True, "id": item_id}
...
async def admin_reorder(
    ...
    """Bulk-update `display_order` for the weekly curation. Body shape:
        { "order": ["<id1>", "<id2>", ...] }
    The first id becomes order 10, the next 20, etc. This drives both
    /api/admin/wishlist-deals and /api/public/wishlist-deals so the admin
    can pick the TOP-9 that appears on the homepage.
    """
    db = _db()
    order = payload.get("order") or []
    if not isinstance(order, list) or not order:
        raise HTTPException(status_code=400, detail="order must be a non-empty list of ids")
    updated = 0
    for i, item_id in enumerate(order):
        if not isinstance(item_id, str) or not item_id:
            continue
        try:
            res = await db.wishlist_deals.update_one(
                {"id": item_id},
                {"$set": {"display_order": (i + 1) * 10}},
            )
            updated += int(res.modified_count or 0)
        except Exception as e:
            logger.warning(f"[wishlist_deals/reorder] {item_id}: {e}")
    return {"updated": updated, "count": len(order)}
...
async def admin_update(
    ...
    """Update budget / note / status / display_order on an existing card."""
    db = _db()
    update: Dict[str, Any] = {}
    if "budget" in payload:
        b = (payload.get("budget") or "").strip()
        if b and b not in VALID_BUDGETS:
            raise HTTPException(status_code=400, detail=f"budget must be one of {sorted(VALID_BUDGETS)}")
        update["budget"] = b
    if "note" in payload:
        update["note"] = (payload.get("note") or "").strip() or None
    if "display_order" in payload:
        try:
            update["display_order"] = int(payload.get("display_order"))
        except Exception:
            raise HTTPException(status_code=400, detail="display_order must be int")
    if "status" in payload:
        ...
        if st == "approved":
            ...
            update["approved_by"] = user.get("id") or user.get("_id") or user.get("user_id")
            update["approved_by_name"] = user.get("name") or user.get("email") or "admin"
    if not update:
        raise HTTPException(status_code=400, detail="Nothing to update")
    res = await db.wishlist_deals.update_one({"id": item_id}, {"$set": update})
    if not res.matched_count:
        raise HTTPException(status_code=404, detail="Item not found")
    doc = await db.wishlist_deals.find_one({"id": item_id})
    return _strip(doc or {})
