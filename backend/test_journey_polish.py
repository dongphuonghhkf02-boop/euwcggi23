...
async def test_socket_throttle(http, ship, db):
    ...
    print("  OK  first tick → emit happened (lastSocketEmitAt set)")
    ...
    print("  OK  second tick within 30s → emit throttled (same lastSocketEmitAt)")
    ...
    if other_stage:
        ...
        print("  OK  stage change bypassed throttle (new lastSocketEmitAt)")
...
async def test_fallback_paths(http, ship, db):
    ...
    print("  OK  with 3h-old real position → source=simulated")
...
async def test_stage_transition_guard(http, ship):
    ...
    print("  OK  pending→done rejected with 400")
    ...
    assert r2.status_code == 200
    print("  OK  label-only edit on pending stage allowed")
...
async def test_eta_smoothing():
    ...
    print("  OK  real_scraped moves ETA faster toward new than simulated")
