...
class PhaseA1Tester:
    ...
    def test_filter_brand_alias(self):
        ...
        if not resp or resp.status_code != 200:
            self.test("VW alias filter request", False, "Request failed", critical=True)
            return
    ...
    def test_land_rover_bug_fix(self):
        ...
        if not resp or resp.status_code != 200:
            self.test("Land Rover filter request", False, "Request failed", critical=True)
            return
    ...
    def test_multi_brand_filter(self):
        ...
        if not resp or resp.status_code != 200:
            self.test("Multi-brand filter request", False, "Request failed", critical=True)
            return
    ...
    def test_brand_model_combo(self):
        ...
        if not resp or resp.status_code != 200:
            self.test("Brand+Model filter request", False, "Request failed", critical=True)
            return
