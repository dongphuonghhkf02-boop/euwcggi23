...
class Wave1Customer360Tester:
    ...
    def login(self):
        ...
        if success and 'access_token' in response:
            ...
            self.log("✅ Logged in successfully", "PASS")
            return True
    ...
    def test_customer_360_endpoint(self):
        ...
        if success:
            ...
            self.log("✅ /360 endpoint structure valid", "PASS")
    ...
    def test_customer_health_endpoint(self):
        ...
        if success:
            ...
            self.log("✅ /health endpoint structure valid", "PASS")
    ...
    def test_customer_health_bulk_endpoint(self):
        ...
        if success:
            ...
            if 'items' not in response:
                self.log("❌ Missing 'items' key in bulk response", "FAIL")
                return False
            ...
            if 'score' not in customer_health or 'segment' not in customer_health:
                self.log("❌ Missing score or segment in bulk response", "FAIL")
                return False
            
            # Should NOT have breakdown or risks (lightweight)
            if 'breakdown' in customer_health or 'risks' in customer_health:
                self.log("⚠️  WARNING: Bulk endpoint should be lightweight (no breakdown/risks)", "WARN")
            ...
            self.log("✅ /customer-health-bulk endpoint structure valid", "PASS")
    ...
    def test_ac11_consistency(self):
        ...
        if len(self.health_scores) < 3:
            self.log("❌ Not enough health data collected for AC#11 test", "FAIL")
            return False
        
        # Compare /360 vs /health
        if self.health_scores['360'] != self.health_scores['health']:
            self.log("❌ AC#11 FAIL: /360 and /health have different scores", "FAIL")
        ...
        if self.health_scores['360'] != self.health_scores['bulk']:
            self.log("❌ AC#11 FAIL: /360 and /bulk have different scores", "FAIL")
            self.log(f"   /360: {self.health_scores['360']}", "FAIL")
        ...
        self.tests_run += 1
        self.tests_passed += 1
        self.log("✅ AC#11 PASS: All endpoints return identical score+segment", "PASS")
    ...
    def test_contracts_endpoint(self):
        ...
        if success:
            ...
            if 'items' not in response or 'total' not in response:
                self.log("❌ Missing 'items' or 'total' in contracts response", "FAIL")
                return False
            
            self.log("✅ /contracts endpoint structure valid", "PASS")
    ...
    def test_documents_crud(self):
        ...
        if 'items' not in response or 'total' not in response:
            self.log("❌ Missing 'items' or 'total' in documents response", "FAIL")
            return False
        ...
        if not success:
            self.log("❌ Failed to upload document", "FAIL")
            return False
        
        doc_id = response.get('document', {}).get('id') or response.get('id')
        if not doc_id:
            self.log("❌ No document ID returned after upload", "FAIL")
            return False
        ...
        if not success:
            self.log("❌ Failed to delete document", "FAIL")
            return False
        
        self.log("✅ DELETE document OK", "PASS")
        return True
    ...
    def test_error_handling(self):
        ...
        if not success:
            self.log("❌ /360 should return 404 for invalid customer ID", "FAIL")
            return False
        ...
        if not success:
            self.log("❌ /health should return 404 for invalid customer ID", "FAIL")
            return False
        ...
        if success:
            ...
            if invalid_id in items:
                self.log("❌ Bulk should not return data for invalid customer ID", "FAIL")
                return False
            self.log("✅ Bulk correctly handles missing IDs (returns empty)", "PASS")
